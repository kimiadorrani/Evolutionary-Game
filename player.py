...
def create_input_layer(player_x, player_y, obstacles):
    try:
        distance0_x = obstacles[0]['x'] - player_x
        distance0_y = obstacles[0]['y'] - player_y
        try:
            distance1_x = obstacles[1]['x'] - player_x
            distance1_y = obstacles[1]['y'] - player_y
        except IndexError:
            distance1_y = (-100) - player_y
            distance1_x = random.randint(177, 430)
    except IndexError:
        distance0_x = random.randint(177, 430)
        distance1_x = random.randint(177, 430)
        distance0_y = (-100) - player_y
        distance1_y = (-100) - player_y

    x = np.array(([distance0_x], [distance0_y], [distance1_x], [distance1_y]),dtype=np.float64)
    normalize(x)
    return x
...
class Player(pygame.sprite.Sprite):
    ...
    def think(self, screen_width, screen_height, obstacles, player_x, player_y):
        ...
        """
        x = create_input_layer( player_x, player_y,obstacles)
